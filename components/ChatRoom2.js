...
const ChatRoom2 = () => {
  ...
  return (
    ...
    <div className="relative flex-1 flex flex-col">
      ...
      <main className="chat-room flex-1 md:flex-none  overflow-x-hidden overflow-y-scroll md:px-4   touch-pan-y">
      ...
      {messages &&
          messages.slice(0, numMessagesToShow).map((message) => (
              <ChatMessage2 key={message.createdAt} message={message} />
              ))}
      ...
      <form
        ...
        className=" border-t border-gray-700 flex w-full  bg-background shadow-lg -mt-4  align-start px-2 md:px-4 py-2"
        >
        <input
          value={formValue}
          onChange={(e) => setFormValue(e.target.value)}
          placeholder="Type message here"
          className=" my-2 justify-center align-middle w-full appearance-none rounded-md border bg-input text-slate-100 border-gray-600 px-3 py-2 placeholder-gray-400 shadow-sm focus:border-green focus:outline-none focus:ring-green sm:text-sm"
          />
